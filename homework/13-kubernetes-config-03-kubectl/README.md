...
**ОТВЕТ:** Для проверки будем использовать **[исправленное приложение](./13-kubernetes-config)** которое теперь нормально работает при открытии наружу только фронтенда (бекенд и база остаются внутри кластера) и **[манифест test_apps.yml](/.test_apps.yml)**.  
